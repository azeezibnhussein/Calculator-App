const e = 2.71828; // Eulers constant
const TOL = 10 ** (-14); // Numerical equivalent to zero
...
function operate(operator, number1, number2 = false) {
    /* Calls the right math function given numbers and operators */
    ...
    else {return "operator use was not valid!";}
}

function check_valid_input(input) {
    /* Check if statment is a valid mathmatical statement */
    ...
    // Loop input element for element and check if its ok
...
function calculate_terms(terms) {
    ...
    for (let i = 0; i < terms.length; i++) {
        ...
        if (operators.length == 0 && numbers.length = 0) {return numbers[0];}
